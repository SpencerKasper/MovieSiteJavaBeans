...
%>
	...
	<div style="text-align:center;">
		<p>
			Are you sure you want to this Item from Order #<c:out value="${order.orderID}"></c:out>?
		</p>
	</div>


	<div style="text-align:center;">
		<form style="display:inline-block; padding:10px;" name="cancellationConfirmationForm" action=CancellationConfirmation  method="post">
				<input class="button" type="submit" value="Confirm Cancellation"></input>
				<input type="hidden" name="OId" value="${order.orderID}">
				<input type="hidden" name="TId" value="${ticket.ticketID}">
		</form>
		
		<form style="display:inline-block; padding:10px;" name="discardCancellationForm" action=ViewOrders method="post">
			<input class="button" type="submit" value="Discard Cancellation">
		</form>
	</div>
</body>
</html>